...
async function fetchMovies(userInput, page = 1) {
  const response = await fetch(
    `https://www.omdbapi.com/?apikey=${apiKey}&s=${userInput}&page=${page}`
  );
...
function renderMovies(movies) {
  resultContanier.innerHTML = movies
    .map(
      (movie) => `<div class="movie-card" data-id="${movie.imdbID}">
      <img src="${
...
async function fetchMovieDetails(imdbID) {
  ...
  if (data.Response === "True") {
    ...
    document.getElementById("modal-metascore").textContent =
      data.Metascore || "N/A";
    document.getElementById("modal-imdbRating").textContent =
      data.imdbRating || "N/A";
    document.getElementById("modal-imdbVotes").textContent =
      data.imdbVotes || "N/A";
